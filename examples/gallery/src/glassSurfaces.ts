...
export const bindGlassSurfaces = (root: HTMLElement) => {
  ...
  >()
  let nextId = 0
  let template: SVGFilterElement | undefined
  let definitions: Element | undefined
  const resolveTemplate = () => {
    if (template && definitions) return true
    const found = root.querySelector('#glass-pill')
    if (!(found instanceof SVGFilterElement) || !found.parentElement) {
      return false
    }
    template = found
    definitions = found.parentElement
    return true
  }
  ...
  const scan = () => {
    ...
    if (!enabled || !template || !definitions) return
  ...
  mutations.observe(root, {
    attributes: true,
    attributeFilter: ['data-theme-pack', 'data-glass-refraction'],
  })
