...
export const obsidianCss = `
  &[data-theme-pack='obsidian'] {
    ...
    --header-inline-pad: 26px;
    #gallery-toolbar {
      min-height: 76px; padding: 18px var(--header-inline-pad); gap: 18px;
    ...
    #gallery-pathbar { min-height: 50px; padding: 8px var(--header-inline-pad); background: var(--bg-secondary); border-bottom: 1px solid var(--border); }
    ...
    [data-gap]:not([data-gap='none']) [data-caption] > div { color: var(--text-secondary); font-size: 10px; letter-spacing: .055em; }
    [data-gap]:not([data-gap='none']) [data-caption] > div:first-child { color: var(--text-primary); font-weight: 400; }
    ...
    [data-gap]:not([data-gap='none']):is(:hover, :focus-within) { border-color: var(--accent); box-shadow: 0 10px 28px #00000040, inset 0 0 12px #bcaed90b; }
    aside[role='dialog'] { border-left: 1px solid var(--border-strong); }
    aside[role='dialog'][data-open='true'] { box-shadow: -20px 0 50px #00000040; }
    ...
    #gallery-lightbox :is(#lightbox-toolbar, #lightbox-filmstrip, #slideshow-controls, #lightbox-scrubber) { border-radius: 2px; backdrop-filter: none; box-shadow: none; }
    ...
    footer[aria-label='Obsidian status'] b { margin: 0 10px; color: var(--accent); font-weight: 400; }
    footer[aria-label='Obsidian status'] i { margin: 0 12px; opacity: .5; }
    @media (max-width: 1100px) {
      --header-inline-pad: 18px;
      #gallery-toolbar { flex-wrap: wrap; gap: 10px; padding: 12px var(--header-inline-pad); }
      #gallery-pathbar { flex-wrap: wrap; gap: 6px; padding: 10px var(--header-inline-pad); }
    }
    @media (max-width: 700px) {
      --header-inline-pad: 12px;
      #gallery-toolbar { gap: 8px; padding: 12px var(--header-inline-pad); }
      #gallery-toolbar > div:first-child { gap: 8px; }
      #gallery-brand > span:first-child { width: 18px; height: 28px; }
      #gallery-folder-sidebar { position: absolute; inset: 0 auto 0 0; z-index: 90; }
      #gallery-folder-sidebar ~ [data-ui] { z-index: 91; }
      ...
      #gallery-pathbar { padding: 8px var(--header-inline-pad); }
      main { padding: 18px 16px; }
      [data-view-mode='grid'] { grid-template-columns: repeat(2, minmax(0, 1fr)); }
      [data-gap]:not([data-gap='none']) { padding: 4px; }
      [data-gap]:not([data-gap='none']) [data-caption] { padding: 10px 2px 6px; }
      [data-gap]:not([data-gap='none']) [data-caption] > div { font-size: 8px; letter-spacing: 0; }
      [data-caption]::after { display: none; }
      footer[aria-label='Obsidian status'] { padding: 10px; font-size: 8px; letter-spacing: 0; }
      footer[aria-label='Obsidian status'] > span:last-child { display: none; }
